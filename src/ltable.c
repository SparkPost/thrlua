...
{
#if LUA_USE_RW_SPINLOCK
  lua_rwspinlock_write_lock(&t->lock);
#else
  int r;
  do {
...
{
#if LUA_USE_RW_SPINLOCK
  lua_rwspinlock_read_lock(&t->lock);
#else
  int r;
  do {
...
{
#if LUA_USE_RW_SPINLOCK
  lua_rwspinlock_write_unlock(&t->lock);
#else
  int r;
  do {
...
{
#if LUA_USE_RW_SPINLOCK
  lua_rwspinlock_read_unlock(&t->lock);
#else
  int r;
  do {
    r = pthread_rwlock_unlock(&t->lock);
